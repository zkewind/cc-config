...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/zkewind/cc-config/releases/latest",
            None::<String>,
        )
...
async fn fetch_github_latest_version(client: &reqwest::Client, repo: &str) -> Option<String> {
    ...
    match client
        .get(&url)
        .header("User-Agent", "claude-code-multi-config")
...
fn try_get_version_wsl(
    ...
    // 防御性断言：tool 只能是预定义的值
    debug_assert!(tool == "claude", "unexpected tool name: {tool}");
...
pub async fn minimize_to_tray(window: tauri::WebviewWindow) -> Result<(), String> {
    crate::tray::hide_main_window_to_tray(&window);
    Ok(())
}
...
mod tests {
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("cc-config-missing-{unique}"));
