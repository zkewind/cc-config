...
pub fn start_worker(db: Arc<crate::json_store::JsonStore>, app: tauri::AppHandle) {
...
async fn run_worker_loop(
    db: Arc<crate::json_store::JsonStore>,
    ...
    while let Some(first_table) = rx.recv().await {
        ...
        if let Err(err) = run_auto_sync_upload(db.as_ref(), &app).await {
