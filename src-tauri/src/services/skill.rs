...
pub enum SkillStorageLocation {
    /// CCConfig 管理目录 (~/.cc-config/skills/)
    #[default]
    CcSwitch,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.cc-config/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.cc-config/skill-backups/）
    ...
    /// 获取所有已安装的 Skills
    pub fn get_all_installed(db: &Arc<JsonStore>) -> Result<Vec<InstalledSkill>> {
    ...
    /// 2. 从 SSOT 删除
    /// 3. 从数据库删除
    pub fn uninstall(db: &Arc<JsonStore>, id: &str) -> Result<SkillUninstallResult> {
        // 获取 skill 信息
        let skill = db
    ...
    /// 按仓库分组下载，避免重复下载同一仓库。
    pub async fn check_updates(&self, db: &Arc<JsonStore>) -> Result<Vec<SkillUpdateInfo>> {
        let skills = db.get_all_installed_skills()?;
    ...
    pub fn backfill_content_hashes(db: &Arc<JsonStore>) -> Result<usize> {
        let skills = db.get_all_installed_skills()?;
        let ssot_dir = Self::get_ssot_dir()?;
        let mut count = 0;
    ...
    /// 启用：复制到应用目录
    /// 禁用：从应用目录删除
    pub fn toggle_app(db: &Arc<JsonStore>, id: &str, app: &AppType, enabled: bool) -> Result<()> {
        // 获取当前 skill
        let mut skill = db
            .get_installed_skill(id)?
            .ok_or_else(|| anyhow!("Skill not found: {id}"))?;
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 CCConfig 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<JsonStore>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "cc-config".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 CCConfig 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "cc-config".to_string()));
    ...
    pub fn sync_to_app(db: &Arc<JsonStore>, app: &AppType) -> Result<()> {
        let skills = db.get_all_installed_skills()?;
        let ssot_dir = Self::get_ssot_dir()?;
        let app_dir = Self::get_app_skills_dir(app)?;
...
/// 首次启动迁移：扫描应用目录，重建数据库
pub fn migrate_skills_to_ssot(db: &Arc<JsonStore>) -> Result<usize> {
