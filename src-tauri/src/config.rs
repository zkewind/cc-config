...
///   且不一定等于用户目录，可能导致 `.cc-config` 配置目录变化，从而"看起来像数据丢失"。
///
/// ## 测试隔离
///
/// 为了让 Windows CI/本地测试能稳定隔离真实用户数据，可通过 `CC_CONFIG_TEST_HOME`
/// 显式覆盖 home dir（仅用于测试/调试场景）。
pub fn get_home_dir() -> PathBuf {
    // 测试隔离：通过该变量显式覆盖 home dir（仅用于测试/调试场景）
    let home_override = std::env::var("CC_CONFIG_TEST_HOME").unwrap_or_default();
    if !home_override.trim().is_empty() {
        return PathBuf::from(home_override.trim());
    }
...
/// 获取应用配置目录路径 (~/.cc-config)
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    get_home_dir().join(".cc-config")
}
