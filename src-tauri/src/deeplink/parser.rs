...
//!
//! Parses ccconfig:// URLs into DeepLinkImportRequest structures.
...
/// Parse a ccconfig:// URL into a DeepLinkImportRequest
///
/// Expected format:
/// ccconfig://v1/import?resource={type}&...
pub fn parse_deeplink_url(url_str: &str) -> Result<DeepLinkImportRequest, AppError> {
    ...
    if scheme != "ccconfig" {
        return Err(AppError::InvalidInput(format!(
            "Invalid scheme: expected 'ccconfig', got '{scheme}'"
        )));
    }
...
fn parse_provider_deeplink(
    ...
    if app != "claude" {
        return Err(AppError::InvalidInput(format!(
            "Invalid app type: must be 'claude', got '{app}'"
        )));
    }
...
fn parse_mcp_deeplink(
    ...
    for app in apps.split(',') {
        ...
        if trimmed != "claude" {
            return Err(AppError::InvalidInput(format!(
                "Invalid app in 'apps': must be 'claude', got '{trimmed}'"
            )));
        }
    }
