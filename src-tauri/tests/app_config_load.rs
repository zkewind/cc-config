...
use cc_config_lib::{AppError, MultiAppConfig};
...
fn cfg_path() -> PathBuf {
    ...
    PathBuf::from(home).join(".cc-config").join("config.json")
}
...
fn load_v1_config_returns_error_and_does_not_write() {
    ...
    let bak = home.join(".cc-config").join("config.json.bak");
...
fn load_valid_v2_config_succeeds() {
    ...
    assert!(loaded
        .get_manager(&cc_config_lib::AppType::Claude)
        .is_some());
}
