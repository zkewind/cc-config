...
use cc_config_lib::{update_settings, AppSettings, AppState, JsonStore, MultiAppConfig};

/// 为测试设置隔离的 HOME 目录，避免污染真实用户数据。
pub fn ensure_test_home() -> &'static Path {
    static HOME: OnceLock<PathBuf> = OnceLock::new();
    HOME.get_or_init(|| {
        let base = std::env::temp_dir().join("cc-config-test-home");
        if base.exists() {
        ...
        // 用 CC_CONFIG_TEST_HOME 显式覆盖，以确保测试不会污染真实用户目录。
        std::env::set_var("CC_CONFIG_TEST_HOME", &base);
...
pub fn reset_test_fs() {
    ...
    for sub in [".claude", ".cc-config", ".config"] {
