...
This directory contains the Flatpak manifest (`com.ccconfig.desktop`) for CCConfig, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
```bash
cp "$(find src-tauri/target/release/bundle -name '*.deb' | head -n 1)" flatpak/claude-code-multi-config.deb
```

3) Build the local Flatpak repository and export the `.flatpak`:

```bash
flatpak-builder --force-clean --user --disable-cache --repo flatpak-repo flatpak-build flatpak/com.ccconfig.desktop.yml
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo claude-code-multi-config-Linux.flatpak com.ccconfig.desktop
```

4) Install and run:

```bash
flatpak install --user ./claude-code-multi-config-Linux.flatpak
flatpak run com.ccconfig.desktop
```
...
If you prefer minimal permissions (e.g., for Flathub submission or security concerns), you can replace `--filesystem=home` in `flatpak/com.ccconfig.desktop.yml` with more precise grants:

```yaml
  - --filesystem=~/.cc-config:create
...
If you plan to publish on Flathub or want stricter permission control, adjust the `finish-args` in `flatpak/com.ccconfig.desktop.yml` accordingly.
