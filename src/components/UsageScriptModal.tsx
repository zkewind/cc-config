...
const generatePresetTemplates = (
  ...
  request: {
    ...
    headers: {
      ...
      "User-Agent": "claude-code-multi-config/1.0"
    }
  },
  extractor: function(response) {
    return {
    ...
    try {
      ...
      if (selectedTemplate === TEMPLATE_TYPES.BALANCE) {
        ...
        if (result.success && result.data && result.data.length > 0) {
          ...
          queryClient.setQueryData(["usage", provider.id, _appId], result);
        } else {
          toast.error(
      ...
      if (selectedTemplate === TEMPLATE_TYPES.TOKEN_PLAN) {
        ...
        if (quota.success && quota.tiers.length > 0) {
          ...
          queryClient.setQueryData(["usage", provider.id, _appId], {
            success: true,
            data: usageData,
          });
        } else {
