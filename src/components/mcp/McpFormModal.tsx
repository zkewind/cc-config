...
const emptyMcpEnabledApps = (): McpEnabledApps => ({
  claude: false,
});
...
const McpFormModal: React.FC<McpFormModalProps> = ({
  ...
  const [enabledApps, setEnabledApps] = useState<McpEnabledApps>(() => {
    if (initialData?.apps) {
      return Object.assign(emptyMcpEnabledApps(), initialData.apps);
    }
    return {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const appsForSave = initialData?.apps
        ? (Object.fromEntries(
            Object.entries(enabledApps).filter(
              ([app, enabled]) => app in initialData.apps || enabled === true,
            ),
          ) as unknown as McpServer["apps"])
        : enabledApps;
